...
class Admin_model extends CI_Model {
    ...
    public function role_edit()
    {
        ...
        if (count($this->error) == 0) {
            ...
            // reset row value
            $row_reset = array();
			$key = array('admin_area','editor_area','thread_create','thread_edit','thread_delete','post_create','post_edit','post_delete','role_create','role_edit','role_delete','faq_create','faq_edit','faq_delete','answerer');
            foreach ($key as $keys) {
                $row_reset[$keys] = 0;
            }
	...
	{
		$this->db->query("UPDATE threads SET publish = 1 WHERE id = ".$id."");
	}
	public function unpub($id)
	{
		$this->db->query("UPDATE threads SET publish = 0 WHERE id = ".$id."");
	}
}